...
- [ ] **T001** `[P]` Crear `src/app/domains/wells/models/well-transition.model.ts`
...
- [ ] **T002** `[P]` Crear `src/app/domains/wells/models/well-transition.dto.ts`
...
- [ ] **T003** `[P]` Crear `src/app/domains/wells/models/well-transition.mapper.ts`
...
- [ ] **T004** Modificar `src/app/shared/models/well.model.ts`
  - Agregar campo: `uwi: string | null`

- [ ] **T005** Modificar `src/app/shared/models/well.dto.ts`
  - Agregar campo: `uwi: string | null`

- [ ] **T006** Modificar `src/app/shared/models/well.mapper.ts`
  - Agregar mapeo: `uwi: dto.uwi ?? null`

- [ ] **T007** Modificar `src/app/domains/wells/models/index.ts`
...
- [ ] **T008** Modificar `src/app/core/http/api-endpoints.ts`
...
- [ ] **T009** Modificar `src/app/domains/wells/services/wells-api.service.ts`
...
- [ ] **T010** Modificar `src/app/domains/wells/mocks/wells.mock.ts`
...
- [ ] **T011** Crear `src/app/domains/wells/components/well-status-badge/well-status-badge.component.ts`
...
- [ ] **T012** Crear `src/app/domains/wells/components/well-status-badge/well-status-badge.component.html`
...
- [ ] **T013** Crear `src/app/domains/wells/features/well-detail/locale.ts`
  - Constante `WELL_DETAIL_LOCALE` con secciones: title, sections, status, actions, dialog, history, uwi, messages
  - Per plan.fe.md §8.1

- [ ] **T014** `[P]` Crear `src/app/domains/wells/features/well-detail/components/well-transition-actions/well-transition-actions.component.ts`
  ...
  - Constante TRANSITION_MATRIX interna: array de `{ from, action, roles[], label, severity }` espejando spec §2.2
  - Standalone, OnPush
  - Imports: PrimeNG `Button`

- [ ] **T015** `[P]` Crear `src/app/domains/wells/features/well-detail/components/well-transition-actions/well-transition-actions.component.html`
...
- [ ] **T016** `[P]` Crear `src/app/domains/wells/features/well-detail/components/well-history-timeline/well-history-timeline.component.ts`
  - Selector: `app-well-history-timeline`
  - Inputs: `history = input.required<TransitionHistoryItem[]>()`, `isLoading = input(false)`
  - Standalone, OnPush
  - Imports: PrimeNG `Timeline`, `Tag`
  - Si `history().length === 0` y `!isLoading()` → muestra mensaje vacío del locale

- [ ] **T017** `[P]` Crear `src/app/domains/wells/features/well-detail/components/well-history-timeline/well-history-timeline.component.html`
...
- [ ] **T018** `[P]` Crear `src/app/domains/wells/features/well-detail/components/transition-confirm-dialog/transition-confirm-dialog.component.ts`
...
- [ ] **T019** `[P]` Crear `src/app/domains/wells/features/well-detail/components/transition-confirm-dialog/transition-confirm-dialog.component.html`
  - Template:
...
- [ ] **T020** Crear `src/app/domains/wells/features/well-detail/well-detail.component.ts`
  ...
  - OnInit:
    - Lee `wellId` de `ActivatedRoute.params`
  ...
  - Método `onTransitionRequested(event: { action })`: abre dialog (dialogVisible=true, dialogAction=action)
  - Método `onTransitionConfirmed(event: { action, comment? })`:
    - isTransitioning=true, dialogVisible=false
    - Llama `wellsApi.transitionWell(wellId, action, comment)`
    - On success: actualiza well signal (estado, uwi), refetch history, toast éxito, isTransitioning=false
    - On error: isTransitioning=false (error manejado por interceptor)
  - Imports: componentes hijos, `WellStatusBadgeComponent`, PrimeNG `Card`, `Button`, `ProgressSpinner`

- [ ] **T021** Crear `src/app/domains/wells/features/well-detail/well-detail.component.html`
  - Layout:
    ...
    - Botón "Editar" visible solo si estado === 'BORRADOR' y rol permite (enlace a `/wells/:id/edit`)
...
- [ ] **T022** Modificar `src/app/domains/wells/wells.routes.ts`
  - Agregar ruta:
    ```typescript
    {
      path: ':id',
    ...
    ```
  - Posicionar **antes** de `:id/edit` si existe, para que Angular resuelva correctamente
...
- [ ] **T023** Modificar template del listado (`src/app/domains/wells/features/well-manage/well-manage.component.html`)
  - En la columna del nombre del pozo en la tabla: envolver en `<a [routerLink]="['/wells', well.id]">` para navegar al detalle
  - Agregar columna de estado con `<app-well-status-badge [estado]="well.estado" />`
  - Agregar columna UWI que muestra `well.uwi ?? '-'`
  - Imports necesarios: agregar `RouterLink` al array de imports del componente TS si no existe

- [ ] **T024** Modificar `src/app/domains/wells/features/well-manage/well-manage.component.ts`
  - Agregar `RouterLink` a imports del componente (si falta)
